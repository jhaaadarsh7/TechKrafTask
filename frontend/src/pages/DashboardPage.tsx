...
//     if (!token || busyPropertyId === propertyId) {
//       return;
//     }
...
//         <div className="rounded-xl border border-stone-300 bg-white px-6 py-4 text-xs font-semibold uppercase tracking-[0.2em] text-stone-500 shadow-sm">
//           Loading dashboard...
//         </div>
//       </main>
//     );
...
//       {/* Header */}
//       <header className="sticky top-0 z-50 animate-fade-in-down border-b border-stone-200 bg-white/95 backdrop-blur shadow-sm">
//         <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
//           <div className="flex items-center gap-3">
//             <h1 className="font-serif text-2xl font-bold text-stone-900">{user?.name}</h1>
//             <span className="rounded-full border border-orange-300 bg-orange-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.1em] text-orange-700">
//               {user?.role}
//             </span>
//           </div>
//           <button
...
//             className="rounded-lg border-2 border-stone-300 bg-stone-50 px-4 py-2 text-xs font-bold uppercase tracking-widest text-stone-700 transition hover:bg-stone-900 hover:text-stone-50 hover:border-stone-900"
//           >
//             Sign Out
//           </button>
//         </div>
...
//         {error ? (
//           <div className="animate-fade-in-up rounded-lg border border-red-300 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700">
//             ✕ {error}
//           </div>
//         ) : null}
//       </div>
...
//             <div className="mb-4">
//               <h2 className="font-serif text-3xl font-bold text-stone-900">All Properties</h2>
//               <p className="mt-1 text-sm text-stone-600">{properties.length} properties available</p>
//             </div>

//             {properties.length === 0 ? (
//               <div className="rounded-2xl border-2 border-dashed border-stone-300 bg-stone-50/50 px-6 py-12 text-center">
//                 <p className="text-sm text-stone-600">No properties available at the moment.</p>
//               </div>
//             ) : (
...
//                       className="group animate-fade-in-up overflow-hidden rounded-2xl border border-stone-200 bg-white/95 shadow-[0_10px_24px_rgba(28,20,8,0.08)] ring-1 ring-stone-100 transition duration-500 hover:-translate-y-1 hover:border-amber-300 hover:shadow-[0_24px_44px_rgba(28,20,8,0.16)]"
//                     >
//                       {/* Image */}
//                       <div className="relative h-48 overflow-hidden bg-stone-200">
//                         <img
//                           src={property.imageUrl ?? "https://images.unsplash.com/photo-1580587771525-78b9dba3b914?w=900&q=80"}
//                           alt={property.title}
//                           className="h-full w-full object-cover transition duration-700 group-hover:scale-110"
//                         />
//                         <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-stone-950/90 to-transparent p-4">
...
//                           className={`absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-full border text-base shadow-md backdrop-blur transition duration-300 disabled:cursor-not-allowed disabled:opacity-50 ${
//                             isFavourite
//                               ? "border-rose-300 bg-rose-100/95 text-rose-700"
//                               : "border-white/80 bg-white/90 text-stone-700 hover:scale-105 hover:border-amber-300 hover:text-amber-700"
//                           }`}
//                         >
//                           {isBusy ? "..." : isFavourite ? "♥" : "♡"}
//                         </button>
...
//                           <h3 className="text-base font-bold text-stone-900 transition group-hover:text-orange-700">
//                             {property.title}
//                           </h3>
//                           <span className="rounded-full border border-stone-200 bg-stone-50 px-2 py-1 text-[10px] font-semibold uppercase tracking-widest text-stone-500">
//                             Premium
//                           </span>
//                         </div>
...
//                           onClick={() => toggleFavourite(property.id)}
//                           disabled={isBusy}
//                           className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-2 text-xs font-bold uppercase tracking-widest transition duration-300 disabled:cursor-not-allowed disabled:opacity-50 ${
//                             isFavourite
...
//                           <span>{isBusy ? "Updating" : isFavourite ? "Liked" : "Like"}</span>
//                         </button>

//                         <div className="mt-3 h-px w-full bg-gradient-to-r from-transparent via-stone-200 to-transparent" />

//                         <p className="mt-3 text-[11px] uppercase tracking-wider text-stone-500">
//                           Tap like to save this property
//                         </p>
//                       </div>
//                     </article>
//                   );
...
//           <aside className="w-full animate-slide-in-right lg:w-80">
//             <div className="sticky top-20 rounded-2xl border border-stone-200 bg-white/95 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.08)] backdrop-blur">
//               <div className="mb-4 border-b border-stone-200 pb-4">
//                 <h3 className="font-serif text-xl font-bold text-stone-900">My Favorites</h3>
//                 <p className="mt-1 text-xs text-stone-600">{favourites.length} saved</p>
//               </div>

//               {favourites.length === 0 ? (
//                 <div className="rounded-lg border border-dashed border-stone-300 bg-stone-50/50 px-4 py-8 text-center">
//                   <p className="text-xs text-stone-600">No favorites yet</p>
//                 </div>
//               ) : (
...
//                       key={item.id}
//                       style={{ animationDelay: `${idx * 0.05}s` }}
//                       className="group animate-fade-in-up rounded-lg border border-stone-200 bg-gradient-to-br from-orange-50/50 to-stone-50 p-3 transition hover:border-orange-300 hover:shadow-md"
//                     >
//                       <h4 className="text-sm font-bold text-stone-900 line-clamp-2">{item.property.title}</h4>
//                       <p className="mt-1 text-xs text-stone-600 line-clamp-1">{item.property.location}</p>
...
//                         className="mt-3 w-full rounded-md border-2 border-red-300 bg-red-50 px-2 py-1.5 text-xs font-bold uppercase tracking-widest text-red-700 transition hover:bg-red-700 hover:text-red-50 disabled:cursor-not-allowed disabled:opacity-50"
//                       >
//                         {busyPropertyId === item.propertyId ? "..." : "Remove"}
//                       </button>
//                     </li>
//                   ))}
//                 </ul>
